...
export function PriceChart() {
  ...
  const { history, isLoading: historyLoading } = useCryptoHistory(selectedCrypto, timeframe)
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            {currentPrice && (
              <div className="flex items-center gap-4">
                <span className="text-2xl font-bold font-mono">${currentPrice.price.toLocaleString()}</span>
                <span className={`font-medium ${currentPrice.change > 0 ? "text-success" : "text-destructive"}`}>
          ...
          <div className="flex gap-2">
            <Button variant={timeframe === 1 ? "default" : "outline"} size="sm" onClick={() => setTimeframe(1)}>
              1D
            </Button>
